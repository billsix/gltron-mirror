 ...
 *
 * $Id: chunk.c,v 1.1 2004/05/31 08:01:29 andi75 Exp $
 */
...
static void lib3ds_chunk_debug_dump(Lib3dsChunk* c) {
  if (enable_dump) {
    printf("%s%s (0x%X) size=%lu\n", lib3ds_chunk_level,
           lib3ds_chunk_name(c->chunk), c->chunk, c->size);
  }
}
 ...
 * \ingroup chunk
 */
Lib3dsBool lib3ds_chunk_read_start(Lib3dsChunk* c, Lib3dsWord chunk,
                                   Lib3dsIo* io) {
  ASSERT(c);
  ASSERT(io);
