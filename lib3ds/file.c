 ...
 *
 * $Id: file.c,v 1.1 2004/05/31 08:01:30 andi75 Exp $
 */
...
static int fileio_read_func(void* self, Lib3dsByte* buffer, int size) {
  FILE* f = (FILE*)self;
  return (fread(buffer, 1, size, f));
}

static int fileio_write_func(void* self, const Lib3dsByte* buffer, int size) {
  FILE* f = (FILE*)self;
...
Lib3dsFile* lib3ds_file_load(const char* filename) {
  ...
  io = lib3ds_io_new(f, fileio_error_func, fileio_seek_func, fileio_tell_func,
...
static Lib3dsBool nodes_write(Lib3dsNode* node, Lib3dsFile* file,
                              Lib3dsIo* io) {
  {
    Lib3dsNode* p;
...
static void dump_instances(Lib3dsNode* node, const char* parent) {
  Lib3dsNode* p;
  char name[255];
 ...
 * \ingroup file
 */
void lib3ds_file_bounding_box(Lib3dsFile* file, Lib3dsVector min,
                              Lib3dsVector max) {
 ...
 * \ingroup file
 */
Lib3dsNode* lib3ds_file_node_by_name(Lib3dsFile* file, const char* name,
