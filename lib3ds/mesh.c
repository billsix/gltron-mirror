 ...
 *
 * $Id: mesh.c,v 1.1 2004/05/31 08:01:30 andi75 Exp $
 */
 ...
 * \ingroup mesh
 */
void lib3ds_mesh_bounding_box(Lib3dsMesh* mesh, Lib3dsVector min,
                              Lib3dsVector max) {
  unsigned i, j;
  Lib3dsFloat v;
...
void lib3ds_mesh_dump(Lib3dsMesh* mesh) {
  ...
  for (i = 0; i < mesh->faces; ++i) {
    printf("    %4d %4d %4d  smoothing:%X\n", mesh->faceL[i].points[0],
           mesh->faceL[i].points[1], mesh->faceL[i].points[2],
...
Lib3dsBool lib3ds_mesh_read(Lib3dsMesh* mesh, Lib3dsIo* io) {
  ...
  }
  {
    ...
    for (j = 0; j < mesh->faces; ++j) {
      ...
      lib3ds_vector_normal(mesh->faceL[j].normal,
...
static Lib3dsBool face_array_write(Lib3dsMesh* mesh, Lib3dsIo* io) {
  ...
  { /*---- MSH_BOXMAP ----*/
    ...
    if (strlen(mesh->box_map.front) || strlen(mesh->box_map.back) ||
        strlen(mesh->box_map.left) || strlen(mesh->box_map.right) ||
        strlen(mesh->box_map.top) || strlen(mesh->box_map.bottom)) {
...
Lib3dsBool lib3ds_mesh_write(Lib3dsMesh* mesh, Lib3dsIo* io) {
  ...
  if (mesh->map_data.maptype !=
      LIB3DS_MAP_NONE) { /*---- LIB3DS_MESH_TEXTURE_INFO ----*/
    Lib3dsChunk c;
    int i, j;
