...
System::~System() {
  ...
  if (_sources.next) nebu_List_Free(_sources.next);
  delete _spec;
}
...
void System::Callback(Uint8* data, int len) {
  ...
  if (_status == eUninitialized) return;
  ...
  for (p = &_sources; p->next != NULL; p = p->next) {
    ...
    if (s->IsPlaying()) {
      ...
      if (!((s->GetType() & eSoundFX && !_mix_fx) ||
        ...
        if (s->Mix(data, len)) sources_mixed++;
      }
...
void System::AddSource(Source* source) { nebu_List_AddTail(&_sources, source); }
