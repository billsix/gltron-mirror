...
nebu_Surface* nebu_Surface_LoadPNG(const char* filename) {
  ...
  switch (pTex->channels) {
    case 1:
      pSurface->format = NEBU_SURFACE_FMT_ALPHA;
      break;
    case 3:
      pSurface->format = NEBU_SURFACE_FMT_RGB;
      break;
    case 4:
      pSurface->format = NEBU_SURFACE_FMT_RGBA;
      break;
    default:
      pSurface->format = NEBU_SURFACE_FMT_UNKNOWN;
      break;
  }
...
static void user_flush_data(png_structp png_ptr) { fflush(fp); }
...
int nebu_Surface_SavePNG(nebu_Surface* pSurface, const char* filename) {
  ...
  png_set_IHDR(png_ptr, info_ptr, pSurface->w, pSurface->h, bitdepth,
               png_format, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT,
...
void nebu_Surface_Blit(nebu_Surface* pSrcSurface, nebu_Rect* pSrcRect,
                       nebu_Surface* pDstSurface, nebu_Rect* pDstRect) {
  nebu_Rect src, dst;
