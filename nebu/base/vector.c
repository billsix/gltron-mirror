...
float vec3_Length(const vec3* pV) {
  return (float)sqrt(pV->v[0] * pV->v[0] + pV->v[1] * pV->v[1] +
                     pV->v[2] * pV->v[2]);
}

float vec3_LengthSqr(const vec3* pV) {
  return pV->v[0] * pV->v[0] + pV->v[1] * pV->v[1] + pV->v[2] * pV->v[2];
}
...
unsigned int uintFromVec3(vec3* pV) {
  return (((unsigned int)(pV->v[0] * 127.0f + 128.0f)) << 16) +
...
vec3* vec3_TriNormalDirection(vec3* pOut, const vec3* pV1, const vec3* pV2,
                              const vec3* pV3) {
  vec3 v1, v2;
  vec3_Sub(&v1, pV2, pV1);
  vec3_Sub(&v2, pV3, pV1);
...
int segment2_findT(float* t, const segment2* s, const vec2* v) {
  ...
  if (fabs(s->vDirection.v[0]) > fabs(s->vDirection.v[1])) {
    *t = (v->v[0] - s->vStart.v[0]) / s->vDirection.v[0];
    if (fabs(v->v[1] - (s->vStart.v[1] + *t * s->vDirection.v[1])) > epsilon) {
      return 1;
    }
  } else {
    *t = (v->v[1] - s->vStart.v[1]) / s->vDirection.v[1];
    if (fabs(v->v[0] - (s->vStart.v[0] + *t * s->vDirection.v[0])) > epsilon) {
      return 1;
    }
  }
  return 0;
...
vec2* segment2_IntersectParallel(vec2* pOut, float* t1, float* t2,
  ...
  if (segment2_findT(t1, s1, &v)) return NULL;
  // if t1 < 0 return NULL (no intersection)
  if (*t1 < 0) return NULL;
  vec2_Add(&v, &s2->vStart, &s2->vDirection);
  if (segment2_findT(&t, s1, &v)) return NULL;
  assert(t >= 0);
...
vec2* segment2_Intersect(vec2* pOut, float* t1, float* t2, const segment2* s1,
  ...
  vec2 tmp;
  if (fabs(vec2_Dot(&s1->vDirection, vec2_Orthogonal(&tmp, &s2->vDirection))) <
      0.1) {
...
float segment2_Length(const segment2* s) { return vec2_Length(&s->vDirection); }

float box2_Width(const box2* pBox) { return pBox->vMax.v[0] - pBox->vMin.v[0]; }
...
void box3_Compute(box3* pBox, const vec3* pVertices, int nVertices) {
  ...
  for (i = 0; i < nVertices; i++) {
    for (j = 0; j < 3; j++) {
      ...
      if (f < pBox->vMin.v[j]) pBox->vMin.v[j] = f;
      if (f > pBox->vMax.v[j]) pBox->vMax.v[j] = f;
    }
  }
}