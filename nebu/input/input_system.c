...
void nebu_Input_Init(void) {
  ...
  if (SDL_Init(SDL_INIT_JOYSTICK) >= 0) {
    ...
    if (NEBU_MAX_JOY) {
      ...
      if (n < 0) n = 0;
      if (n > 4) n = 4; /* this is the max we can handle! */
      if (!*endptr && !errno) max_joy = n;
    }

    if (joysticks > max_joy) joysticks = max_joy;

    for (i = 0; i < joysticks; i++) {
      joy = SDL_JoystickOpen(i);
    }
    if (i) SDL_JoystickEventState(SDL_ENABLE);
  } else {
...
void nebu_Intern_HandleInput(SDL_Event* event) {
  ...
  switch (event->type) {
    ...
    case SDL_MOUSEBUTTONUP:
      SystemMouse(event->button.button, event->button.state, event->button.x,
                  event->button.y);
      break;
