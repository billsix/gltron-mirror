...
void nebu_Mesh_DrawGeometry(nebu_Mesh* pMesh) {
  ...
  glDrawElements(GL_TRIANGLES, 3 * pMesh->nTriangles, GL_UNSIGNED_INT,
                 pMesh->pTriangles);
...
void nebu_Mesh_ComputeTriangleNormal(nebu_Mesh* pMesh, int triangle,
                                     float* normal) {
  float v1[3], v2[3];
  int a, b, c, i;
...
void nebu_Mesh_Free(nebu_Mesh* pMesh) {
  ...
  if (pMesh->vertexformat & NEBU_MESH_POSITION) free(pMesh->pVertices);
  if (pMesh->vertexformat & NEBU_MESH_NORMAL) free(pMesh->pNormals);
