...
png_texture* load_png_texture(const char* filename) {
  ...
  for (i = 0; i < y; i++) row_pointers[i] = tex->data + (y - i - 1) * zsize * x;
...
#define SOURCE(X, Y, I) \
  (source->data + I + source->channels * (X + Y * source->width))

png_texture* mipmap_png_texture(png_texture* source, int level, int clamp_u,
                                int clamp_v) {
  png_texture* mip;
  int i;
  int x, y;
  ...
  mip->data = (unsigned char*)malloc(mip->width * mip->height * mip->channels);
    ...
    for (y = 0; y < mip->height; y++) {
      for (x = 0; x < mip->width; x++) {
        ...
        *(mip->data + i + mip->channels * (x + y * mip->width)) =
            (*(source->data + i +
               source->channels * (sx + sy * source->width)) +
             *(source->data + i +
               source->channels * (sx + xoff + sy * source->width)) +
             *(source->data + i +
               source->channels * (sx + xoff + (sy + yoff) * source->width)) +
             *(source->data + i +
               source->channels * (sx + (sy + yoff) * source->width))) /
            4;
      }
    }
  }
