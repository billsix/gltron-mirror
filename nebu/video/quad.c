...
int getCurrentQuad(QuadBuffer* q) { return q->current; }

int getQuadBufferSize(QuadBuffer* q) { return q->size; }

void q_setType(Quad* q, int type) { q->type = type; }

Quad* getQuadAt(QuadBuffer* q, int index) { return q->quads + index; }
