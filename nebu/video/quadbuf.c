...
int occlusiontest(float* x, float* y, float* p, float* a, float* b) {
  ...
  vsub2(ti, x, nt);
  if (scalarprod2(n1, nt) < 0) return 1;
...
void checkWall(float* eye, QuadBuffer* q, int i, int* pos, int* index,
               int* processed) {
  float x[2], y[2];
  float a[2], b[2];
  int j;
...
int* getSortedQuads(QuadBuffer* q, float eye[3]) {
  ...
  for (i = 0; i < q->current; i++) processed[i] = 0;

  for (i = 0; i < q->current; i++)
    if (processed[i] == 0) checkWall(eye, q, i, &pos, index, processed);
