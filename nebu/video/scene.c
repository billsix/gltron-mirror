...
void nebu_Scene_AddObject(nebu_Scene* pScene, nebu_Shader* pShader,
                          nebu_Mesh* pMesh) {
...
void nebu_Scene_Draw(nebu_Scene* pScene) {
  ...
  for (l = &pScene->objectList; l->next; l = l->next) {
    ...
    if (pObject->pShader->setup) (pObject->pShader->setup)();
    if (pObject->pMesh) nebu_Mesh_DrawGeometry(pObject->pMesh);
    if (pObject->pShader->cleanup) (pObject->pShader->cleanup)();
  }
