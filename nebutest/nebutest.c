...
void mouseMotion(int x, int y) {
  ...
  if (dx || dy) {
    nebu_Camera_Rotate(pScene->pCamera,
                       NEBU_CAMERA_ROTATE_AROUND_EYE | NEBU_CAMERA_FIXED_UP,
                       (float)dx, (float)dy);
...
void solidShaderStart(void) { glColor3f(1, 1, 1); }

void wireFrameSetup(void) {
  glColor3f(1, 0, 0);
  glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
}
void wireFrameCleaup(void) { glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); }
...
int main(int argc, char* argv[]) {
  ...
  nebu_Video_SetDisplayMode(SYSTEM_32_BIT | SYSTEM_RGBA | SYSTEM_DOUBLE |
                            SYSTEM_STENCIL | SYSTEM_DEPTH);
