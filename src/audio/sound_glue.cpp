...
void Audio_EnableEngine(void) {
  ...
  for (i = 0; i < game->players; i++)
    if (game->player[i].data->speed > 0) Audio_StartEngine(i);
  sample_engine->Start();
  if (gSettingsCache.show_recognizer) sample_recognizer->Start();
...
void Audio_LoadMusic(char* name) {
  ...
  music->Load(name);
  if (getSettingi("loopMusic")) music->SetLoop(255);
...
void Audio_PlayMusic(void) { music->Start(); }

void Audio_StopMusic(void) { music->Stop(); }

void Audio_SetMusicVolume(float volume) { music->SetVolume(volume); }
...
void Audio_StartEngine(int iPlayer) { players[iPlayer]->Start(); }

void Audio_StopEngine(int iPlayer) { players[iPlayer]->Stop(); }
