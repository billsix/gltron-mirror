...
static void clampCam(Camera* cam) {
  if (cam->type.freedom[CAM_FREE_R]) {
    if (cam->movement[CAM_R] < CLAMP_R_MIN) cam->movement[CAM_R] = CLAMP_R_MIN;
    if (cam->movement[CAM_R] > CLAMP_R_MAX) cam->movement[CAM_R] = CLAMP_R_MAX;
  }
...
void initCamera(Camera* cam, Data* data, int type) {
  ...
  switch (cam->type.type) {
    case CAM_TYPE_CIRCLING:
      initCircleCamera(cam);
      break;
    case CAM_TYPE_FOLLOW:
      initFollowCamera(cam);
      break;
    case CAM_TYPE_COCKPIT:
      initCockpitCamera(cam);
      break;
    case CAM_TYPE_MOUSE:
      initFreeCamera(cam);
      break;
    case CAM_TYPE_OFFSET:
      initOffsetCamera(cam);
      break;
  }
  cam->target[0] = x;
  cam->target[1] = y;
...
void playerCamera(Player* p, int player) {
  ...
  if (game->pauseflag != PAUSE_GAME_RUNNING ||
      !getSettingi("mouse_lock_ingame")) {
    if (cam->type.freedom[CAM_FREE_R]) {
      // mouse buttons let you zoom in/out
      if (gInput.mouse1 == 1)
        cam->movement[CAM_R] +=
            (cam->movement[CAM_R] - CLAMP_R_MIN + 1) * dt / 300.0f;
      if (gInput.mouse2 == 1)
        cam->movement[CAM_R] -=
            (cam->movement[CAM_R] - CLAMP_R_MIN + 1) * dt / 300.0f;
  ...
  if (cam->type.coupled) {
    ...
    if (time < TURN_LENGTH) {
      ...
      if (dir == 1 && ldir == 2) dir = 4;
      if (dir == 2 && ldir == 1) ldir = 4;
      phi += ((TURN_LENGTH - time) * camAngles[ldir] + time * camAngles[dir]) /
             TURN_LENGTH;
    } else
  ...
  switch (cam->type.type) {
    ...
    case CAM_TYPE_OFFSET: /* AMR-cam */
    {
      ...
      dx = px[player] - tx;
      dy = py[player] - ty;
      d = 1.3f * sqrtf(dx * dx + dy * dy); /* Find distance between old
                                              viewpoint and scaled cycle pos */
      px[player] = dest[0] =
          tx +
          CAM_CIRCLE_DIST * dx / d; /* Set viewpoint a fixed distance from */
      py[player] = dest[1] =
          ty + CAM_CIRCLE_DIST * dy / d; /* scaled pos, preserving angle */
      ...
      dest[2] =
          CAM_CIRCLE_Z -
          (d * CAM_CIRCLE_Z) / 24; /* ...and lift camera when close to cycle */
