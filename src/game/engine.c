...
void initGameLevel(void) {
  if (game2->level) game_FreeLevel(game2->level);
...
void resetPlayerData(void) {
  ...
  for (i = 0; i < game->players; i++) {
    ...
    switch (i) {
      ...
      case 0:
        ai->active = getSettingi("ai_player1");
        break;
      case 1:
        ai->active = getSettingi("ai_player2");
        break;
      case 2:
        ai->active = getSettingi("ai_player3");
        break;
      case 3:
        ai->active = getSettingi("ai_player4");
        break;
      default:
...
void resetScores(void) {
  int i;
  for (i = 0; i < game->players; i++) game->player[i].data->score = 0;
}
