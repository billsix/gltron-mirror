...
#endif
      if (e->player < PLAYERS &&
          game->player[e->player].ai->active != AI_NONE) {
...
int crashTestPlayers(int i, const segment2* movement) {
  ...
  for (j = 0; j < game->players; j++) {
    ...
    if (game->player[j].data->trail_height < TRAIL_HEIGHT) continue;
    ...
    for (k = 0; k < game->player[j].data->trailOffset + 1; k++) {
      ...
      if (j == i && k >= game->player[j].data->trailOffset - 1) break;
...
int regenerate(int player, int dt) {
  float curEnergy = game->player[player].data->energy +
                    getSettingf("energy_increase") * dt / 1000.0f;
...
int applyBooster(int player, int dt) {
  ...
  if (!data->boost_enabled) return 0;
...
void applyDecceleration(int player, int dt, float factor) {
  ...
  if (data->speed > game2->rules.speed) {
    ...
    data->speed -= decrease;
    if (data->speed < game2->rules.speed) data->speed = game2->rules.speed;
  }
}
...
void doMovement(int dt) {
  ...
  for (i = 0; i < game->players; i++) {  // foreach player
    ...
    if (data->speed > 0) {  // still alive
      ...
      if (deccel > 0) applyDecceleration(i, dt, deccel);

      // if(i == 0)
      // printf("speed: %.2f, boost: %.2f\n", data->speed, data->energy);

      fs = 1.0f - SPEED_OZ_FACTOR +
           SPEED_OZ_FACTOR *
               cosf(i * PI / 4.0f + (game2->time.current % SPEED_OZ_FREQ) *
...
void Game_Idle(void) {
  ...
  while (dt > 0) {
    ...
    // chop time since last frame into ticks of maximally PHYSICS_RATE
    // milliseconds
    if (dt > PHYSICS_RATE)
      t = PHYSICS_RATE;
    else
    ...
    /*
    game_ComputeMovement(dt); // compute new speed, energy levels, movement
    vectors game_HandleMovement(dt); // handle movement based events
    game_AdjustMovement(dt); // adjust movement vectors due to collision
    response game_AdjustWorld(dt); // write back changes to world
    */
    game_RunAI(dt);
    /*
    ...
    */
    ...
    for (p = &(game2->events); p->next != NULL; p = p->next) {
      if (processEvent((GameEvent*)p->data))
        ...
        // TODO: fix, because this leaks the list element, and also all
        // unprocessed turn events!
        return;
    }
  ...
  doCameraMovement();
  {
    ...
    if (mouse_dx != 0 || mouse_dy != 0) nebu_Input_Mouse_WarpToOrigin();
  }
