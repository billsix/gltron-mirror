...
void idlePause(void) {
  ...
  doCameraMovement();
  {
    ...
    if (dx || dy) nebu_Input_Mouse_WarpToOrigin();
  }
  scripting_RunGC();
...
Callbacks pauseCallbacks = {displayGame, idlePause, keyboardPause, initPause,
                            exitPause,   gameMouse, NULL,          "pause"};
...
Callbacks promptCallbacks = {displayGame,
                             idlePause,
                             keyboardPrompt,
                             initPrompt,
                             exitPrompt,
