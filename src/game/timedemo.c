...
void keyTimedemo(int state, int key, int x, int y) {
  ...
  if (key == 27) nebu_System_ExitLoop(eSRC_Timedemo_Abort);
}
...
void exitTimedemo(void) {
  ...
  if (dt) {
    displayMessage(TO_STDERR | TO_CONSOLE, "timedemo FPS: %.2f\n",
                   (float)frames / dt * 1000.0f);
    // displayMessage(TO_STDERR | TO_CONSOLE, "timedemo FPS: %.2f (%d frames in
    // %f seconds)\n", (float) frames / dt * 1000.0f, frames, dt / 1000.0f);
  } else {
    // displayMessage(TO_STDERR | TO_CONSOLE, "dt: %d, frames: %d\n", dt,
    // frames); actually, this would be a good reason to abort with a fatal
    // error
  }
...
Callbacks timedemoCallbacks = {displayGame,
                               idleTimedemo,
                               keyTimedemo,
                               initTimedemo,
                               exitTimedemo,
