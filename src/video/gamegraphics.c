...
void clearScreen() {
  glClearColor(gSettingsCache.clear_color[0], gSettingsCache.clear_color[1],
               gSettingsCache.clear_color[2], 0);
...
#define neigung 25
  if (time < TURN_LENGTH && p->data->last_dir != p->data->dir) {
    ...
    if (p->data->dir < p->data->last_dir && p->data->last_dir != 3)
      ...
      axis = -1.0;
    glRotated(neigung * sin(PI * time / TURN_LENGTH), 0.0, axis * neigung_dir,
              0.0);
  }
#undef neigung
}
...
void drawCycleShadow(PlayerVisual* pV, Player* p, int lod, int drawTurn) {
  ...
  if (turn_time < TURN_LENGTH && !drawTurn) return;

  if (p->data->exp_radius != 0) return;

  cycle =
      (gltron_Mesh*)resource_Get(gpTokenLightcycles[lod], eRT_GLtronTriMesh);
...
int playerVisible(int eyePlayer, int targetPlayer) {
  ...
  /*
          ...
          s, d);
  */
  if (s < d - (((gltron_Mesh*)resource_Get(gpTokenLightcycles[i],
                                           eRT_GLtronTriMesh))
                   ->BBox.fRadius *
               2))
    return -1;
...
void drawPlayers(int player) {
  ...
  for (i = 0; i < game->players; i++) {
    ...
    if (i == player && gSettingsCache.camType == CAM_TYPE_COCKPIT) drawTurn = 0;
...
void drawPlanarShadows(int player) {
  ...
  for (i = 0; i < game->players; i++) {
    ...
    if (lod >= 0) {
      int drawTurn =
          (i != player || gSettingsCache.camType != CAM_TYPE_COCKPIT) ? 1 : 0;
  ...
  if (gSettingsCache.use_stencil) glDisable(GL_STENCIL_TEST);
...

  {
    ...
    for (i = 0; i < game->players; i++) {
      if (game->player[i].data->trail_height > 0) {
        ...
        int iOffset = 0;
        mesh.iUsed = 0;
        trailGeometry(game->player + i, gPlayerVisuals + i, &mesh, &vOffset,
                      &iOffset);
        bowGeometry(game->player + i, gPlayerVisuals + i, &mesh, &vOffset,
                    &iOffset);
...
static float getReflectivity() {
  ...
  if (reflectivity < 0) reflectivity = getVideoSettingf("reflection");

  // need stencil for reflections
  if (gSettingsCache.use_stencil == 0) reflectivity = 0;
  return reflectivity;
}
...
void drawCam(int player) {
  ...
  doPerspective(gSettingsCache.fov, (float)d->vp_w / (float)d->vp_h,
                ...
                box2_Diameter(&game2->level->boundingBox) * 6.5f);
  ...
  if (gSettingsCache.show_glow == 1) {
    ...
    for (i = 0; i < game->players; i++) {
      if (i != player && PLAYER_IS_ACTIVE(game->player + i)) {
        drawGlow(&gPlayerVisuals[player].camera, game->player + i,
                 gPlayerVisuals + i, d, TRAIL_HEIGHT * 4);
      }
    }
    glDisable(GL_BLEND);
  }
