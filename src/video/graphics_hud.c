...
void hud_MaskFinish(void) { glDisable(GL_STENCIL_TEST); }
...
void drawHUD(Player* p, PlayerVisual* pV) {
  ...
  */
  ...
  sprintf(
      ...
      gSettingsCache.show_ai_status ? (p->ai->active ? "AI_COMPUTER" : "") : "",
      p->data->speed, p->data->speed / (2 * game2->rules.speed),
      p->data->energy / getSettingf("energy"),
      0.0f,  // formerly wallbuster
      getFPS(), pause_message, pause_color[0], pause_color[1], pause_color[2]);
...
void getPauseString(char* buf, float* color) {
  ...
  if ((game->pauseflag & PAUSE_GAME_FINISHED) && game->winner != -1) {
    if (game->winner >= -1) {
       ...
       */
      color[0] =
          ((player_color[0] + ((sinf(d) + 1) / 2) * (1 - player_color[0])));
      color[1] =
...
void drawCircle(float phiStart, float phiEnd, int nSegments, float r1, float r2,
...
int c_drawCircle(lua_State* l) {
  ...
  lua_pop(l, 1);
  nSegments = (int)lua_tonumber(l, -1);
  lua_pop(l, 1);
  phiEnd = (float)lua_tonumber(l, -1);
  lua_pop(l, 1);
  phiStart = (float)lua_tonumber(l, -1);
  lua_pop(l, 1);
...
void drawCircle(float phiStart, float phiEnd, int nSegments, float r1, float r2,
  ...
  pIndices = (unsigned int*)malloc((nSegments) * 2 * 3 * sizeof(unsigned int));
...
void drawRect(float width, float height, float* colors) {
  float vertices[12] = {0, 0, 0, 0, height, 0, width, height, 0, width, 0, 0};
