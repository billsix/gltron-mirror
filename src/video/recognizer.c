...
static float x(void) {
  return xv[0] * sinf(xv[1] * alpha + xv[2]) -
         xv[3] * sinf(xv[4] * alpha + xv[5]);
}
static float y(void) {
  return yv[0] * cosf(yv[1] * alpha + yv[2]) -
         yv[3] * sinf(yv[4] * alpha + yv[5]);
}

static float dx(void) {
  return xv[1] * xv[0] * cosf(xv[1] * alpha + xv[2]) -
         xv[4] * xv[3] * cosf(xv[4] * alpha + xv[5]);
}
static float dy(void) {
  return -yv[1] * yv[0] * sinf(yv[1] * alpha + yv[2]) -
         yv[4] * yv[3] * sinf(yv[4] * alpha + yv[5]);
}
...
void drawRecognizerShadow(void) {
  ...
  gltron_Mesh_Draw(
      ((gltron_Mesh*)resource_Get(gTokenRecognizer, eRT_GLtronTriMesh)),
      TRI_MESH);
...
void drawRecognizer(void) {
  ...
  gltron_Mesh_Draw(
      ((gltron_Mesh*)resource_Get(gTokenRecognizerQuad, eRT_GLtronQuadMesh)),
      QUAD_MESH);
...
void doRecognizerMovement(void) { alpha += game2->time.dt / 2000.0f; }

void resetRecognizer(void) { alpha = 0; }
