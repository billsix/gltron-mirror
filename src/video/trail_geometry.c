...
enum { COLOR_TRAIL, COLOR_BRIGHT, COLOR_CYCLE };
...
void storeVertex(TrailMesh* pMesh, int offset, segment2* s,
                 float t, /* 0: start, 1: end */
                 float fFloor, float fTop, float fSegLength,
                 float fTotalLength) {
...
void storeIndices(TrailMesh* pMesh, int indexOffset, int vertexOffset) {
  unsigned short ppBase[2][6] = {{0, 2, 1, 2, 3, 1}, {0, 1, 2, 1, 3, 2}};
  int i;
  int winding;
...
void trailGeometry(Player* pPlayer, PlayerVisual* pV, TrailMesh* pMesh,
  ...
  for (i = 0; i < pData->trailOffset; i++) {
    ...
    if (i == 0 || cmpdir(pData->trails + i - 1, pData->trails + i)) {
      storeVertex(pMesh, curVertex, pData->trails + i, 0, 0,
                  pData->trail_height, fSegLength, fTotalLength);
      storeColor(pMesh, curVertex, pV, COLOR_TRAIL);
      curVertex += 2;
    }

    storeVertex(pMesh, curVertex, pData->trails + i, 1, 0, pData->trail_height,
  ...
  }
  {
    ...
    storeVertex(pMesh, curVertex, &s, 1, 0, pData->trail_height, fSegLength,
                fTotalLength);
    storeColor(pMesh, curVertex, pV, COLOR_TRAIL);
    curVertex += 2;
...
void bowGeometry(Player* pPlayer, PlayerVisual* pV, TrailMesh* pMesh,
                 int* pvOffset, int* piOffset) {
  Data* pData = pPlayer->data;
  segment2 s;
  ...
  for (i = 0; i < 10; i++) {
    ...
    storeVertex(pMesh, vOffset, &s, t, fFloor * pPlayer->data->trail_height,
                fTop * pPlayer->data->trail_height, DECAL_WIDTH, 0);
  ...
  storeVertex(pMesh, vOffset, &s, 1, 0.2f * pData->trail_height,
              0.3f * pData->trail_height, DECAL_WIDTH, 0);
