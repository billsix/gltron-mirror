...
int initWindow(void) {
  ...
  if (getSettingi("windowMode") == 0) flags |= SYSTEM_FULLSCREEN;
...
void reshape(int x, int y) {
  if (x < getSettingi("height") || x < getSettingi("width")) initGameScreen();
  if (x > getSettingi("width")) gScreen->vp_x = (x - getSettingi("width")) / 2;
...
void video_LoadLevel(void) {
  ...
  if (gWorld) video_FreeLevel(gWorld);
  gWorld = video_CreateLevel();
  if (gWorld->scalable) video_ScaleLevel(gWorld, getSettingf("grid_size"));
}
...
void updateDisplay(int vpType) {
  ...
  for (i = 0; i < vp_max[vpType]; i++) {
    initDisplay(&gPlayerVisuals[viewport_content[i]].display, vpType, i, 1);
  }
}
