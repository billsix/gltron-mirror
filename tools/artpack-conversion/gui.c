...
void render() {
  ...
  glPushMatrix();
  glTranslatef(width / 512.0f * (512 - 320 - 10), height / 384.0f * (384 - 80),
               0);
...
void convertGUI(void) {
  ...
  if (!nebu_FS_Test("gltron.png") || !nebu_FS_Test("gltron_logo.png")) return;
...
void convertWalls(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!nebu_FS_Test(buf)) return;

    pWalls[i] = nebu_Surface_LoadPNG(buf);
  }
  pResult =
      nebu_Surface_Create(4 * pWalls[0]->w, pWalls[0]->h, pWalls[0]->format);
  for (i = 0; i < 4; i++) {
    nebu_Rect r;
    r.width = pWalls[0]->w;
    r.height = pWalls[1]->h;
